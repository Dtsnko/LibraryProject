...
namespace SoftServeProject.Services
{
    public class ManagerService : IManagerService
    {
        ...
        public void RegisterBookCopy(string title, string name, string surname)
        {
            ...
            {
                auth = new List<Author> { new Author() { Name = name, Surname = surname } };
                context.Authors.AddRange(auth);

            }
            book = new Book { Title = title, Authors = auth.ToList() };
